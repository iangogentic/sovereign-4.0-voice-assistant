#!/usr/bin/env python3
"""
Simple conversational voice assistant - FIXED VERSION
Fixed VAD threshold for better speech detection
"""
...
class SimpleConversationalAssistant:
    """Simple conversational voice assistant with fixed VAD threshold"""
    ...
    async def initialize(self):
        ...
        try:
            ...
            # STT setup with FIXED VAD threshold
            stt_config = STTConfig(
                model="whisper-1", 
                language="en",
                silence_threshold=0.001,  # 🔧 FIXED: Lowered from 0.01 to 0.001
                vad_enabled=True,
                min_audio_length=0.3      # 🔧 FIXED: Lowered from 0.5 to 0.3 seconds
            )
    ...
    async def listen(self, duration: float = 5.0):
        """Listen for user input with better VAD settings"""
        try:
            ...
            logger.info(f"Audio captured: RMS={normalized_rms:.4f}, Max={max_amplitude}")
            
            # Convert to text
            stt_result = await self.stt_service.transcribe_audio(audio_data)
            if stt_result and stt_result.text.strip():
                logger.info(f"Successfully transcribed: '{stt_result.text}'")
    ...
    async def run(self):
        ...
        try:
            print("🎤 Voice Assistant Started! (FIXED VERSION)")
            print("💬 I'll greet you and then we can have a conversation")
            print("🔧 Fixed VAD threshold for better speech detection")
